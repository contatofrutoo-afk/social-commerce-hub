  ...
  if (error) {
    if (error.code === "42703" || error.message?.includes("column") || error.message?.includes("does not exist")) {
      const { error: fbErr } = await supabase.from("company_admin").upsert(
        { company_id: companyId, [field]: value } as any,
...
function EditableTextCell({ row, field, label }: { row: any; field: string; label?: string }) {
  ...
  const cancel = useCallback(() => { setEditing(false); setValue(row[field] ?? ""); }, [row, field]);

  useClickOutside(ref, cancel);

  const save = async () => {
    if (value === (row[field] ?? "")) { setEditing(false); return; }
    setSaving(true);
    try {
      ...
      <div ref={ref} className="flex items-center gap-1">
        ...
        {saving ? (
          ...
          <>
            ...
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={cancel}><X className="h-3 w-3" /></Button>
          </>
        )}
      </div>
    );
  ...
  return (
    <button
      ...
      onClick={() => { setValue(row[field] ?? ""); setEditing(true); }}
    >
      <span>{row[field] ?? "—"}</span>
...
function EditableNumberCell({ row, field }: { row: any; field: string }) {
  ...
  const cancel = useCallback(() => { setEditing(false); setValue(String(Number(row[field] ?? 0).toFixed(2))); }, [row, field]);

  useClickOutside(ref, cancel);

  const save = async () => {
    const num = parseFloat(value.replace(",", "."));
    if (isNaN(num) || num === Number(row[field])) { setEditing(false); return; }
    setSaving(true);
    try {
  ...
  return (
    <button
      ...
      onClick={() => { setValue(String(Number(row[field] ?? 0).toFixed(2))); setEditing(true); }}
    >
...
function EditableDateCell({ row, field }: { row: any; field: string }) {
  ...
  useEffect(() => { if (editing && inputRef.current) { inputRef.current.focus(); inputRef.current.showPicker?.(); } }, [editing]);
...
function EditableSelectCell({
  ...
  useClickOutside(ref, () => { if (editing) setEditing(false); });

  const save = async (newValue: string) => {
    if (newValue === row[field]) { setEditing(false); return; }
    setSaving(true);
    try {
  ...
  if (editing) {
    return (
      <div ref={ref} className="flex items-center gap-1">
        <Select value={value} onValueChange={(v) => { setValue(v); save(v); }}>
