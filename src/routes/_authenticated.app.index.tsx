...
import type { PaymentVerificationOrder } from "@/repositories";
...
function DashboardPage() {
  ...
  const [confirmTarget, setConfirmTarget] = useState<PaymentVerificationOrder | null>(null);
  const seenPaymentRef = useRef<Set<string>>(new Set());
  const [pulseIds, setPulseIds] = useState<Set<string>>(new Set());

  const verifiedFromISO = useMemo(() => {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
    return d.toISOString();
  }, []);

  const { data: awaitingPayments } = useQuery({
    queryKey: ["payments-awaiting", companyId],
    queryFn: () => orderRepository.listAwaitingVerification(companyId!),
    enabled: !!companyId,
  });

  const { data: verifiedPayments } = useQuery({
    queryKey: ["payments-verified", companyId, verifiedFromISO],
    queryFn: () => orderRepository.listVerifiedToday(companyId!, verifiedFromISO),
    enabled: !!companyId,
  });
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`payments-realtime-${companyId}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "orders", filter: `company_id=eq.${companyId}` },
        () => {
          qc.invalidateQueries({ queryKey: ["payments-awaiting", companyId] });
          qc.invalidateQueries({ queryKey: ["payments-verified", companyId] });
        },
      )
      .subscribe();
    return () => {
  ...
  // Animação suave apenas para pagamentos que chegaram com o bloco aberto.
  useEffect(() => {
    if (!awaitingPayments) return;
    const fresh: string[] = [];
    awaitingPayments.forEach((p) => {
      if (!seenPaymentRef.current.has(p.id)) {
        seenPaymentRef.current.add(p.id);
        fresh.push(p.id);
      }
    });
    if (fresh.length > 0) {
      setPulseIds(new Set(fresh));
      const t = setTimeout(() => setPulseIds(new Set()), 10000);
  ...
  const confirmReceipt = useMutation({
    ...
    onError: () => {
      toast.error("Erro ao confirmar o recebimento.");
      setConfirmTarget(null);
    },
  });
    ...
    <div className="dash-surface -m-6 space-y-6 p-6 pb-8 md:-m-6 md:p-6">
      ...
      <section className="dash-card p-5">
        <div className="flex flex-wrap items-center gap-2">
          <span className="grid size-8 place-items-center rounded-xl bg-green-500/10 text-green-600">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Confirmados pelo Mercado Pago — confira o recebimento.
            </p>
          </div>
          <span className="ml-auto inline-flex items-center gap-1.5 rounded-full bg-green-500/10 px-2.5 py-0.5 text-xs font-semibold text-green-700">
              ...
              {verifiedPayments.map((p) => (
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-3 text-xs text-muted-foreground">
                    <span className="number-display font-semibold">{formatBRL(p.total)}</span>
                    <span>{timeOnly(p.verifiedAt)}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmar recebimento deste pagamento?</AlertDialogTitle>
            <AlertDialogDescription>
              {confirmTarget ? (
                <span>
                  ...
                  · Pedido #{confirmTarget.id.slice(0, 6).toUpperCase()} ·{" "}
                  {formatBRL(confirmTarget.total)}
                </span>
              ) : null}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={(e) => {
                ...
                e.preventDefault();
                confirmReceipt.mutate(confirmTarget.id);
              }}
            >
              Confirmar
            </AlertDialogAction>
...
function PaymentCard({
  ...
  payment: PaymentVerificationOrder;
  pulsing: boolean;
  onConfirm: () => void;
}) {
    ...
    >
      ...
      <div className="mt-2 grid grid-cols-2 gap-x-2 gap-y-1 text-xs sm:grid-cols-4">
        ...
        <div>
          ...
          <div className="number-display font-bold text-green-700">{formatBRL(payment.total)}</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Horário</div>
          <div className="font-semibold">{timeOnly(payment.paymentApprovedAt)}</div>
        </div>
        <div>
