-- ============================================================
-- SISTEMA UNIVERSAL DE "OPÇÕES DO PRODUTO"
--
-- Grupos de opções por produto + valores + seleções congeladas
-- por item de pedido (snapshot no momento da compra).
--
-- Tipos de opção:
--   single    -> escolha única (radio)  — preço adicional por valor
--   multiple  -> escolha múltipla (checkbox) — preço adicional por valor
--   text      -> texto livre (sem preço)
--   quantity  -> quantidade com preço por unidade (ex.: extra bacon)
--   toggle    -> ligar/desligar com preço fixo (ex.: adicionar cheddar)
--
-- Seguro reexecutar.
-- ============================================================

-- 1. Grupos de opções do produto
...
-- 2. Valores das opções (usado por single / multiple)
...
-- 3. Seleções congeladas por item de pedido (snapshot)
...
-- RLS: opções e valores seguem o mesmo padrão de product_media —
-- membros da empresa gerenciam, qualquer um pode ler.
...
-- order_item_options: leitura para membros da empresa (via order).
-- As leituras do cliente passam por RPC SECURITY DEFINER (list_customer_orders).
...
-- ============================================================
-- 2) get_product_public: inclui as opções (com valores) no retorno
-- ============================================================
...
-- ============================================================
-- 3) create_customer_order: recebe as opções por item, recalcula o
--    preço unitário de forma AUTORITATIVA (produto + opções do banco)
--    e grava o snapshot das seleções em order_item_options.
--
--    Contrato do item: { productId, quantity, note?,
--      options: [
--        { optionId, valueId? | freeText? | quantity? }
--      ] }
--      - single/multiple -> um row por valor selecionado (valueId)
--      - text            -> freeText
--      - quantity/toggle -> quantity (0 = não selecionado)
-- ============================================================
...
BEGIN
  ...
  FOR it IN SELECT * FROM json_array_elements(_items) LOOP
    ...
    -- Validação de grupos single/multiple (contagem de seleções).
    ...
    -- Preço unitário autoritativo: produto + ajustes das opções selecionadas.
    v_unit := v_base;
    FOR v_opt IN SELECT * FROM json_array_elements(COALESCE(it->'options', '[]'::json)) LOOP
      v_opt_id := (v_opt->>'optionId')::uuid;
...
-- ============================================================
-- 4) list_customer_orders: devolve as opções congeladas por item
-- ============================================================
...
-- ============================================================
-- 5) list_public_posts: produtos marcados ganham slug e has_options
--    (o feed usa para abrir /p/:slug quando o produto tem opções)
-- ============================================================
