-- ============================================================================
-- WEAZE — FINANCEIRO DA PLATAFORMA (script único para colar no SQL Editor)
--
-- Garante que o painel admin mostre SOMENTE pagamentos concluídos:
--   * Mercado Pago = apenas pagamentos APROVADOS de verdade no gateway
--     (orders.status = 'payment_approved' — marcado só pelo webhook/checkout
--     quando o Mercado Pago responde approved);
--   * No Caixa = pedidos finalizados/concluídos no balcão.
-- Pagamentos apenas "gerados" (preferência criada, nunca paga) NÃO aparecem.
--
...
-- ----------------------------------------------------------------------------
-- 5) CONFERÊNCIA — rode e veja o que o painel vai exibir.
--    Esperado: Mercado Pago = somente pagamentos aprovados no gateway;
