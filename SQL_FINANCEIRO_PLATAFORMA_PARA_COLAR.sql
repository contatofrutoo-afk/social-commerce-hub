-- ============================================================================
-- FINANCEIRO DA PLATAFORMA — SCRIPT ÚNICO PARA COLAR NO SQL EDITOR DO SUPABASE
--
-- Cria (se não existir) a infraestrutura do painel admin Financeiro da
-- Plataforma e popula com TODAS as vendas reais já pagas:
--
--   1. Tabela platform_payments + índices + RLS (só admin lê).
--   2. RPCs finalize_order / complete_order (registram pagamento "No Caixa").
--   3. Backfill de vendas online do Mercado Pago (origem mercado_pago) e
--      de vendas no caixa (origem cashier) que já existiam antes.
--
-- Idempotente: seguro rodar quantas vezes quiser. Registros já criados pelo
-- webhook (com net_received_amount real) NÃO são sobrescritos.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 2) RPCs — registram o pagamento "No Caixa"
-- ----------------------------------------------------------------------------

-- finalize_order: pedido sem mesa (loja/QR/link geral) finalizado no caixa.
...
-- complete_order: pedido de mesa concluído pelo fluxo de avanço de status.
...
-- ----------------------------------------------------------------------------
-- 3) BACKFILL — vendas reais que já existiam antes
-- ----------------------------------------------------------------------------

-- 3.1 Mercado Pago: pedidos pagos online (o painel reflete o valor total real
--     do pedido; o webhook, a partir de agora, grava net_received_amount real).
INSERT INTO public.platform_payments
  (company_id, company_name, order_id, payment_origin, payment_method,
...
-- 3.2 No Caixa: pedidos concluídos sem gateway.
INSERT INTO public.platform_payments
  (company_id, company_name, order_id, payment_origin, payment_method,
   payment_status, gross_amount, net_amount, paid_at)
...
-- ----------------------------------------------------------------------------
-- CONFERÊNCIA (opcional) — rode e veja o que o painel vai exibir:
--   SELECT payment_origin, payment_status, count(*), sum(gross_amount)
